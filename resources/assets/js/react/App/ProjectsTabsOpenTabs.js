    ...
    return (
      <React.Fragment>
        {tabs.map((tab, index) => {
          ...
          const isTabHover = (tabHover === index)
          return (
            <Tab
              key={index}
              isActiveTab={isActiveTab}
              isFirstTab={index === 0}
              isTabHover={isTabHover}
              onMouseEnter={() => this.setState({ tabHover: index })}
              onMouseLeave={() => this.setState({ tabHover: null })}>
              <ChangeTab
...
const Tab = styled.div`
  ...
  cursor: pointer;
  height: 100%;
  background-color: ${props => props.isActiveTab ? colors.siteBackground : "rgb(40, 44, 55)"};
  ...
  &:hover {
    background-color: ${props => props.isActiveTab ? colors.siteBackground : colors.tabHoverColor};
  }
  &:after {
    height: 100%;
    width: 100%;
